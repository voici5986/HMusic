...
/**
 * 音乐搜索功能 - 同步版本
 * @param {string} source - 音源平台 (kw, kg, tx, wy, mg)  
 * @param {string} keyword - 搜索关键词
 * @param {number} page - 页码，从1开始
 * @returns {Array} 搜索结果数组
 */
const handleSearch = (source, keyword, page = 1) => {
  try {
    // 平台代码映射
    const platformMap = {
      'qq': 'tx',
    ...
    const mappedSource = platformMap[source.toLowerCase()] || source
    const t = 'music'
    const requestBody = { keyword: String(keyword || ''), page: Number(page) || 1, limit: 30 }
    const b64 = handleBase64Encode(JSON.stringify(requestBody)).replace(/\+/g, '-').replace(/\//g, '_')
    const candidates = (function(ms){
      var s = (ms||'').toLowerCase()
      if (s==='tx' || s==='qq' || s==='tencent') return ['tx','qq','txqq']
      if (s==='wy' || s==='netease' || s==='163') return ['wy','netease','163']
      if (s==='kw' || s==='kuwo') return ['kw','kuwo']
      if (s==='kg' || s==='kugou') return ['kg','kugou']
      if (s==='mg' || s==='migu') return ['mg','migu']
      return [ms]
    })(mappedSource)
    ...
    for (var ci=0; ci<candidates.length; ci++) {
      ...
      const searchUrl = `${API_URL}/search/${src}/${t}?q=${b64}`
      console.log(`[LocalJS] 开始同步搜索: ${searchUrl}`)
      
      // 使用同步请求 - 直接调用底层的request函数
      let result = null
      let completed = false
      let error = null
      ...
      while (!completed && attempts < maxAttempts) {
        attempts++
        for (let i = 0; i < 100000; i++) { /* 空循环 */ }
      }
      
      if (!completed) {
        console.log(`[LocalJS] 搜索请求超时: ${searchUrl}`)
        continue
      }
      if (error) {
        console.log(`[LocalJS] 搜索请求失败: ${error}`)
        continue
      }
      const body = (result||{}).body
      if (!body || isNaN(Number(body.code))) {
        console.log(`[LocalJS] 无效的响应体: ${JSON.stringify(body)}`)
        continue
      }
      if (Number(body.code) === 0) {
        const results = (body.data?.list || body.data?.result || body.data || [])
        console.log(`[LocalJS] 搜索成功: ${src}/${keyword}，找到 ${results.length} 个结果`)
        if (results && results.length) {
          finalResults = results
          break
        }
      } else {
        console.log(`[LocalJS] 源(${src}) 搜索失败: code=${body.code}, msg=${body.msg}`)
        continue
      }
    }
    ...
    while (!completed && attempts < maxAttempts) {
      attempts++
      // 简单的忙等待，在JavaScript环境中比较有效
      for (let i = 0; i < 100000; i++) { /* 空循环 */ }
    }
    
    return finalResults
  } catch (e) {
    console.log(`[LocalJS] handleSearch异常: ${e}`)
    return []
  }
}

// 导出搜索函数以供Flutter客户端调用
globalThis.search = handleSearch
globalThis.musicSearch = handleSearch
globalThis.searchMusic = handleSearch
